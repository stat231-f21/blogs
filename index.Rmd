...
```{r blogdata, cache = FALSE, eval = FALSE}
...
blogdata <- readxl::read_xlsx("blog-data.xlsx") %>% 
  mutate(team = str_replace_all(repo, "(-)", " ") %>% str_to_title(),
         ...
         title = map_chr(link, ~ read_html(.x) %>% 
                                         ...
                                         html_text()),
         title = ifelse(is.na(title), "No html file yet", title),
...
<iframe height="500" width="100%" frameborder="no" src="https://bebailey.shinyapps.io/lab07a_shiny-baby-names/"> </iframe>

```{r, out.extra = "height='500' width='100%' frameborder='no'", eval = TRUE}
